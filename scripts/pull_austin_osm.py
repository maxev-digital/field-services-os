"""
Austin Metro — OSM Overpass pull
Travis (Austin), Williamson (Round Rock, Georgetown, Cedar Park),
Hays (Kyle, Buda, San Marcos). 5x5 grid for Travis, 4x4 for others.

Run: python3 pull_austin_osm.py
"""
import json, time, logging, hashlib, urllib.parse, urllib.request
import psycopg2
from psycopg2.extras import execute_values

logging.basicConfig(
    level=logging.INFO, format="%(asctime)s %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler("/tmp/austin_osm.log")]
)
...
COUNTIES = {
    "travis":     {"bbox": (30.00, -98.20, 30.65, -97.40), "grid": 5},
    "williamson": {"bbox": (30.40, -98.05, 31.00, -97.25), "grid": 4},
    "hays":       {"bbox": (29.70, -98.35, 30.35, -97.60), "grid": 4},
}
...
def pull_county(county_key):
    ...
    for idx, (s, w, n, e) in enumerate(cells, 1):
        log.info(f"  [{idx}/{len(cells)}] ({s:.3f},{w:.3f},{n:.3f},{e:.3f})")
...
if __name__ == "__main__":
    for county in COUNTIES:
        pull_county(county)
    log.info("All Austin metro counties done.")
