/**
 * Public Storm Forecast API — no auth, CORS enabled for roofworksoftexas.com
 * Mirrors /api/admin/storm/forecast/route.ts logic exactly, minus requireAdmin().
 */
...
async function fetchAlerts() {
  try {
    const res = await fetch(
      ...
      {
        headers: {
          'User-Agent': 'RoofWorksPublic/1.0 (info@roofworksoftexas.com)',
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { alerts, outlooks: [day1, day2, day3], forecast },
